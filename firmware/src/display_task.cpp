...
PB_SmartKnobState g_dis_state = {0};
...
void DisplayTask::run() {
    ...
    ledcSetup(LEDC_CHANNEL_LCD_BACKLIGHT, 10000, 7);
    ...
    while (1) {
        // tft_.fillScreen(TFT_RED);
        // if (xQueueReceive(knob_state_queue_, &g_dis_state, portMAX_DELAY) == pdTRUE) {
        if (1 == g_dis_state.new_flag) {
            g_dis_state.new_flag = 0;

            spr_.fillSprite(TFT_BLACK);

            int32_t num_positions = g_dis_state.config.max_position - g_dis_state.config.min_position + 1;
            if (num_positions > 1) {
                int32_t height = (g_dis_state.current_position - g_dis_state.config.min_position) * TFT_HEIGHT / (g_dis_state.config.max_position - g_dis_state.config.min_position);
                spr_.fillRect(0, TFT_HEIGHT - height, TFT_WIDTH, height, FILL_COLOR);
            }

            spr_.setFreeFont(&Roboto_Light_60);
            spr_.drawNumber(g_dis_state.current_position, TFT_WIDTH / 2, TFT_HEIGHT / 2 - VALUE_OFFSET, 1);
            spr_.setFreeFont(&DESCRIPTION_FONT);
            int32_t line_y = TFT_HEIGHT / 2 + DESCRIPTION_Y_OFFSET;
            char *start = g_dis_state.config.text;
            char *end = start + strlen(g_dis_state.config.text);
            while (start < end) {
                ...
                char buf[sizeof(g_dis_state.config.text)] = {};
            ...
            if (num_positions > 0) {
                float range_radians = (g_dis_state.config.max_position - g_dis_state.config.min_position) * g_dis_state.config.position_width_radians;
            ...
            float adjusted_sub_position = g_dis_state.sub_position_unit * g_dis_state.config.position_width_radians;
            if (num_positions > 0) {
                if (g_dis_state.current_position == g_dis_state.config.min_position && g_dis_state.sub_position_unit < 0) {
                    adjusted_sub_position = -logf(1 - g_dis_state.sub_position_unit * g_dis_state.config.position_width_radians / 5 / PI * 180) * 5 * PI / 180;
                }
                else if (g_dis_state.current_position == g_dis_state.config.max_position && g_dis_state.sub_position_unit > 0) {
                    adjusted_sub_position = logf(1 + g_dis_state.sub_position_unit * g_dis_state.config.position_width_radians / 5 / PI * 180) * 5 * PI / 180;
                }
            }

            float raw_angle = left_bound - (g_dis_state.current_position - g_dis_state.config.min_position) * g_dis_state.config.position_width_radians;
            float adjusted_angle = raw_angle - adjusted_sub_position;

            if (num_positions > 0 && ((g_dis_state.current_position == g_dis_state.config.min_position && g_dis_state.sub_position_unit < 0) || (g_dis_state.current_position == g_dis_state.config.max_position && g_dis_state.sub_position_unit > 0))) {
